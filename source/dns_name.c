...
int dns_name_packet(stralloc *out,const char *buf,unsigned int len)
{
  ...
  while (numanswers--) {
    ...
    if (dns_packet_rrtypematch(rrfixed,DNS_T_PTR))
...
int dns_name(stralloc *out,const struct ip_address * ip)
{
  char name[DNS_NAME_DOMAIN];

  if (dns_name_domain(name,ip) < 0) return -1;
