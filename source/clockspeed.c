...
int main()
{
  ...
  if (fifo_make("adjust",0600) == -1) if (errno != error_exist) return (1);
  if (open_read("adjust") != 0) return (1);
  if (open_write("adjust") == -1) return (1);
